...
* **learn:** stop classifying a successful `exit code 0` as an error. `is_error_content` listed the bare substring `"exit code"` as an error indicator, so the `exit code 0` that agent harnesses (Codex, Grok, opencode, ...) append to every successful shell command was flagged as a failure — inflating the per-project failure rate the digest reports to the LLM and skewing loop detection. It now matches a *nonzero* exit code (case-insensitive, so `Exit code: 1` counts too) and never `exit code 0`.
