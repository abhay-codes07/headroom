...
* **proxy/memory:** don't crash the memory tool adapter on a null `function` or `arguments`. `_get_tool_name` / `_get_tool_id` / `_get_tool_input` read `tool_call.get("function", {}).get(...)`, so an explicit `{"function": null}` (emitted by upstreams for a partial/streamed tool call) yielded `None` and the following `.get` raised `AttributeError`; separately, a null `arguments` made `json.loads(None)` raise `TypeError` that the bare `JSONDecodeError` catch missed. Both parse untrusted upstream responses. They now coalesce `function`/`functionCall` with `or {}` and the arguments string with `or "{}"`, and also catch `TypeError`.
