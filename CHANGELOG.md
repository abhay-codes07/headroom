...
* **proxy/memory:** don't crash memory tool-call detection on a null `function`. `MemoryHandler.has_memory_tool_calls`, `handle_tool_calls`, and the tool-injection dedup read `tool_call.get("function", {}).get("name")`, which raises `AttributeError` on an explicit `{"function": null}` (the default only applies to a missing key) — a shape upstreams emit for a partial/streamed tool call. All three now coalesce `function` with `or {}`.
