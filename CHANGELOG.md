...
* **proxy/openai:** don't crash the OpenAI Responses memory tool loops on a null `arguments`. Both memory tool-execution loops read `fc.get("arguments", "{}")`, but the `"{}"` default only applies to a missing key: a `function_call` item with `"arguments": null` makes the value `None`, and `json.loads(None)` raises `TypeError` — which the surrounding `except json.JSONDecodeError` does not catch. The arguments string is now coalesced with `or "{}"` and the `except` also catches `TypeError`, matching `parse_tool_call` which already guards this.
