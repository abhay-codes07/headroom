...
* **proxy/batch:** don't crash an OpenAI batch on a valid-JSON non-object line. `_compress_batch_jsonl` parsed each JSONL line and immediately called `request_obj.get("body", {})`, but a line that is valid JSON yet not an object (a stray array, string, or `null`) — or a request whose `body` isn't a dict — makes `.get` raise `AttributeError`, which the surrounding `except json.JSONDecodeError` does not catch, so one such line aborted compression for the entire batch. Those lines now pass through unchanged, like the other non-compressible cases.
