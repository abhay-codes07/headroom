...
* **proxy/streaming:** preserve non-standard content-block fields when reconstructing an SSE response. `_parse_sse_to_response` special-cased `text` / `tool_use` / `thinking` / `redacted_thinking` and collapsed every other block type (`server_tool_use`, `web_search_tool_result`, ...) to a bare `{type, index}`, dropping its `id` / `name` / `input` / content. The reconstructed response feeds memory- and CCR-tool-call detection, so a stream that used a server-side tool lost that block's payload. Non-standard blocks now copy through all their fields, matching the sibling `_reconstruct_anthropic_response` (which uses `dict(block)`).
