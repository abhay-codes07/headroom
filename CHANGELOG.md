...
### Fixed
- **proxy/openai:** feed `/v1/chat/completions` traffic into the live traffic learner ([#2060](https://github.com/headroomlabs-ai/headroom/issues/2060)). The learner was wired into the Anthropic `/v1/messages` and OpenAI Responses HTTP handlers, but `handle_openai_chat` had no ingestion call site, so OpenAI-compatible chat clients (GitHub Copilot CLI, opencode, OpenAI SDKs) ran through an apparently healthy proxy with Learn enabled while producing no learned patterns. `handle_openai_chat` now observes the original client payload (before compression) via a new `_observe_openai_chat_traffic`, mirroring the Responses path. Because chat/completions encodes tool calls differently from Anthropic — the call is on an assistant message's `tool_calls` array and each result is a separate `role: "tool"` message keyed by `tool_call_id` — a new `TrafficLearner.extract_tool_results_from_openai_messages` resolves the tool name from the call-id map and parses the `arguments` JSON string into a dict so the downstream environment/recovery extractors keep working. User-message preference extraction already worked on either format. (Codex WebSocket ingestion, which additionally needs transcript-replay baselining, remains a follow-up.)
