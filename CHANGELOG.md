...
### Fixed
- **backends/litellm:** coerce a `None` `completion_tokens` to `0` when mapping LiteLLM usage to Anthropic shape. `_anthropic_usage_from_litellm` guards `prompt_tokens` and both cache fields with `int(... or 0)`, but `output_tokens` was taken as a bare `getattr(litellm_usage, "completion_tokens", 0)`. LiteLLM's `Usage` always carries the attribute, so the `getattr` default never fires; a provider that leaves it `None` therefore produced `output_tokens=None`, which flowed through the buffered Anthropic-backend response into `RequestOutcome.output_tokens` (declared `int`) and crashed the outcome-recording path (e.g. Prometheus `tokens_output_total += output_tokens` raises `TypeError` on `None`). The field is now `None`-guarded like its siblings.
