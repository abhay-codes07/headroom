...
* **proxy/bedrock:** report uncached input tokens from the backend usage instead of the live-zone token count. On the buffered Anthropic-backend path (Bedrock / Vertex / LiteLLM-anthropic) the handler set `uncached_input_tokens = attempted_input_tokens - cache_read - cache_creation`, but `attempted_input_tokens` is the small live-zone tokenizer count kept for the compression-ratio denominator, not the full request size. On any multi-turn request whose cached prefix is larger than the new turn, that subtraction underflowed and `max(0, …)` reported `uncached=0` for real uncached input — skewing cost/uncached metrics. It now uses `usage.input_tokens`, which the backend already computes as `prompt − cache_read − cache_creation` (see `_anthropic_usage_from_litellm`, #1345), matching the direct-API path.
