...
* **proxy/openai:** forward compressed Codex WS `/v1/responses` frames instead of always the raw bytes. In the per-frame compression path the "compression failed → forward original with a `compression_exception` reason" `return` was dedented to the enclosing `try/except`'s own level, so it ran on the **success** path too: every frame returned the uncompressed bytes and the entire compression-success block below it (the `modified` check, the `store: false` rewrite, and the `return rewritten, True, "compressed"`) was unreachable dead code. So Codex WS frame compression saved nothing and `headroom_codex_ws_frames_compressed` stayed at zero. The failure `return` now lives inside the `except`, so a successfully compressed frame is forwarded.
