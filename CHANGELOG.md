...
### Fixed
- **backends/litellm:** None-guard the core token counts in the OpenAI-shape usage block from `send_openai_message`. `prompt_tokens`, `completion_tokens`, and `total_tokens` were copied straight off LiteLLM's `Usage` object, while the cache fields right below already used `int(getattr(..., 0) or 0)`. A provider that leaves any of those `None` produced a `None` in `response.body["usage"]`; the backend-routed OpenAI handler then reads `output_tokens = usage.get("completion_tokens", 0)` and `total_input_tokens = usage.get("prompt_tokens", ...)` (a present key returns its `None`, not the default) and uses them in `RequestOutcome(output_tokens=...)` and `max(0, total_input_tokens - cache_read - cache_write)`, which raise `TypeError` on `None`. The counts are now coerced to `int` like the sibling fields.
