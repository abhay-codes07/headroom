...
* **learn/gemini:** stop double-counting session tokens. `_parse_messages` added `cachedContentTokenCount` on top of `promptTokenCount` (the cached count is already a subset of the prompt count) and added `totalTokenCount - promptTokenCount` on top of `candidatesTokenCount` (that difference just equals the candidates count again), inflating both the input and output token totals for every Gemini session with usage metadata. It now counts `promptTokenCount` as input and `candidatesTokenCount` as output, once each.
