...
### Fixed
- **learn:** stop treating proxy-injected memory context and role-less Responses items as user preference evidence ([#2274](https://github.com/headroomlabs-ai/headroom/issues/2274)). `TrafficLearner._extract_preferences` already stripped `<system-reminder>` blocks, but Headroom's own injected `## Relevant Memories` block — which is appended to the latest user turn and echoes previously-learned `User preference:` lines — still fed the correction scanner, so the learner re-ingested its own memories as fresh evidence, a self-reinforcing loop that inflates evidence counts. A new `_strip_injected_memory_context` drops everything from the `## Relevant Memories` header to end-of-text before extraction, so a genuine correction preceding the block is still learned while the injected echoes are not. Separately, `_responses_input_to_learner_messages` promoted a Responses item with a missing/blank `role` to `role="user"`, letting ambiguous harness scaffolding reach the user-role preference gate; such items are now labelled `unknown` (fails closed for preference learning, still available to the role-agnostic tool-result extraction). Broader strict-envelope classification (heartbeats, orchestration wrappers) remains a follow-up.
