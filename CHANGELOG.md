...
- **transforms/adaptive-sizer:** honor `max_k` on the small-input fast path. `compute_optimal_k` documents `max_k` as "never return more than this", and the near-duplicate and standard tiers both clamp to it, but the `n <= 8` fast path returned the raw item count and ignored the cap. A caller that passed a tight budget (e.g. `max_k=5`) on a small list (6-8 items) got back more items than it asked for, over-filling the downstream compression budget. The fast path now returns `min(n, effective_max)`, matching the other tiers.
