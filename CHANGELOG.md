...
* **learn/gemini:** detect the project path for JSONL sessions. `_detect_project_path` read the session with a whole-file `json.load`, which raises on the second line of a `.jsonl` stream (Gemini CLI's newer format), so detection returned `None` and `discover_projects` silently fell back to `Path.cwd()` — attributing the learned insights to the wrong project and failing to find its `GEMINI.md`. It now reads JSONL line-by-line (mirroring the sibling `_scan_jsonl_session` and the Claude plugin's cwd recovery) and pulls `projectPath` / `cwd` from whichever record carries it.
