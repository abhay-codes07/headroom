...
### Fixed
- **transforms/kompress-remote:** keep `RemoteKompressCompressor.compress` fail-open on a malformed 200 response. The class promises to fail open ("any network/HTTP error returns the content verbatim"), but only the network call and the `compressed` field check ran inside the guard; the `int()`/`float()`/`str()` coercions of `original_tokens`, `compressed_tokens`, `compression_ratio`, and `model_used` ran after the `except`, outside it. A hosted endpoint that returned a 200 with a valid `compressed` string but a malformed metadata field, e.g. an explicit JSON `null` (`data.get` returns `None` for a present key, so `float(None)` raises) or a non-numeric string, raised out of `compress` and broke the proxy request instead of passing through. The response parsing now happens inside the fail-open `try`, so any bad field degrades to verbatim passthrough.
