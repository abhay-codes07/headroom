...
* **learn/claude:** don't abort the whole scan on a `{"message": null}` line. The Claude session scanner read `d.get("message", {}).get(...)` in four places, but the `{}` default only applies to a missing key — an explicit `"message": null` (which can appear in a session log) yielded `None` and the following `.get` raised `AttributeError`, which the per-file `OSError`/`UnicodeDecodeError` guard does not catch, so one malformed line crashed the entire `headroom learn` run. All four sites now coalesce the message with `or {}`.
