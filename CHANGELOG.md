...
### Fixed
- **proxy/openai:** preserve the Responses content-part shape when losslessly compacting an excluded tool output ([#2235](https://github.com/headroomlabs-ai/headroom/issues/2235)). For an excluded tool (e.g. `grep`) whose `function_call_output.output` is a list of content parts, the adapter joined every part's text, compacted the joined string, and wrote it back through the whole-value `("output", None)` slot. That replaced the entire array with a plain string, so part count, order, and types were lost and non-text parts (images, other metadata) were dropped. The excluded fold now compacts each text part in place via an `("output_part", index)` slot; a list `output` stays a list and only its eligible `output_text`/`input_text` fields change. A plain string `output` still uses the whole-value slot as before.
