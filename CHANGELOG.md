...
* **memory:** correct the `_EMBEDDER_CACHE` type annotation after the Ollama base-URL cache-key change. The key gained a third element (`ollama_base_url`) but the dict was still annotated `dict[tuple[str, str], Embedder]`, so `mypy` failed on the `.get(key)` and `[key] = embedder` sites. Annotate it as `dict[tuple[str, str, str], Embedder]` to match the runtime key. No behavior change.
