...
### Fixed
- **proxy/openai:** apply output shaping on `/v1/chat/completions` ([#2302](https://github.com/headroomlabs-ai/headroom/issues/2302)). `HEADROOM_OUTPUT_SHAPER=1` verbosity steering was wired into the Anthropic `/v1/messages` and OpenAI `/v1/responses` handlers but never into `handle_openai_chat`, so OpenAI-compatible clients that route through chat/completions (GitHub Copilot CLI, opencode, older SDKs) got zero output savings and `headroom output-savings` reported "No shaped requests recorded yet." The chat handler now runs the same holdout/stratum assignment and, for the treatment arm, injects the byte-stable steering block. Chat carries the system prompt as a `role: "system"` message rather than a top-level `system`/`instructions` field, so a new `apply_openai_chat_verbosity_steering` appends (or idempotently replaces) the block on the last system/developer message, inserting one only when the request has no system message. Effort routing is intentionally not applied on this path (its levers are Anthropic-shaped with no portable chat equivalent); only the token-reducing verbosity lever runs. Disabled by default and skipped under `x-headroom-bypass`.
