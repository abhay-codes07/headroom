...
* **memory/hnsw:** raise the query-time `ef` to cover the candidate count so search doesn't crash on a larger index. `HNSWVectorIndex.search` requests `min(top_k * 10, size)` candidates but left `ef` at the configured `ef_search` (default 50). hnswlib requires `ef >= k`, so with the default `top_k=10` any index holding more than ~50 entries made `knn_query` raise "Cannot return the results in a contigious 2D array. Probably ef or M is too small" — HNSW-backed vector search broke as soon as the store grew. Search now sets `ef = max(ef_search, k)` for the query.
