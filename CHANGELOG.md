...
* **learn/grok:** detect a Windows absolute project path. `discover_projects` decoded the URL-encoded workspace cwd and treated it as absolute only when it started with `/`, so a Windows drive-letter path (e.g. `C:\Users\...`) failed the check and silently fell back to `Path.cwd()` — attributing the learnings to the wrong project and missing its `GROK.md` / `AGENTS.md`. It now uses `Path(decoded).is_absolute()`, which recognises drive-letter paths, mirroring the Windows-aware path handling in `memory/traffic_learner.py`.
