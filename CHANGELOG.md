...
* **ccr:** stop crashing on a valid OpenAI 200 response with an empty `choices: []`. `CCRResponseHandler._extract_tool_calls` / `_extract_assistant_message` read `response.get("choices", [{}])[0]` — the default only fires when the key is *absent*, so a present-but-empty `choices: []` (Azure OpenAI content-filter blocks, or the trailing usage-only chunk when `stream_options.include_usage` is set) raised `IndexError` and turned a benign upstream response into a proxy 500. The OpenAI branches now use `response.get("choices") or [{}]`, matching the empty-safe Anthropic branch.
