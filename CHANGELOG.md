...
* **ccr:** don't crash CCR tool-call detection on a null `function` / `functionCall`. `is_ccr_tool_call` and `parse_tool_call` read `tool_call.get("function", {}).get(...)`, but the `{}` default only applies to a missing key: an explicit `{"function": null}` (which upstreams emit for a partial or streamed tool call) yields `None`, and the following `.get` raised `AttributeError` while parsing the upstream response. Both now coalesce with `or {}`, matching the null-safe pattern the sibling `tool_call_id_for_provider` already uses.
