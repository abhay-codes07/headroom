...
* **memory:** make explicit-project-id and USER-mode store keys collision-resistant. The project resolver's `_identity_from_cwd` already appends a `sha256` digest to the sanitized basename, but the explicit `x-headroom-project-id` tier and USER-mode routing used the bare sanitized basename as the key. Since `_sanitize_basename` maps every disallowed character to a dash, distinct ids like `acme/api` and `acme api` (or user ids `alice/qa` and `alice qa`) collapse to the same basename and were pooled into one `memory.db` — a cross-project, and in USER mode cross-user, memory leak. Both now append the same digest of the raw id, keeping distinct ids on distinct stores while the sanitized prefix stays human-readable. Note: this re-keys existing explicit-project and USER stores; pre-existing memories under the old bare-basename paths are not migrated.
