...
* **savings:** don't fabricate output-shaping savings for a free (zero-priced) model. `_estimate_output_savings_usd` read `output_cost_per_token` from litellm and used `if not output_cost_per_token: raise`, which treats a legitimate `0.0` (a free / local / vendored-at-0 model) as "price unavailable" and falls back to `DEFAULT_FALLBACK_OUTPUT_COST_PER_TOKEN` ($15/M) — reporting phantom output savings for a model that costs nothing. It now falls back only when the price is truly missing (`is None`), mirroring the fix already applied to `_estimate_compression_savings_usd` / `_estimate_input_cost_usd`.
