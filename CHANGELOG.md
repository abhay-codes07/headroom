...
* **proxy/openai:** fix the Responses WebSocket memory-relay control flow. In the Codex WS relay the Phase-1 "flush buffer + record metrics + reset" block sat at Phase-1's own indentation, so it ran on *every* upstream event instead of only on `response.completed`. Consequences: the per-response state was reset every event (so memory-tool suppression never took effect and the tool's `output_item` events leaked to the client instead of being executed transparently), `_record_ws_response_metrics()` was called once per event (massively over-counting a turn's tokens), and the suppress / pass-through phases below were unreachable dead code. The flush/record/reset now lives inside the `response.completed` branch and Phase 1 ends with an unconditional `continue`, matching the reference relay pinned by `tests/test_ws_memory_relay.py`.
