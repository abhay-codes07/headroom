...
* **transforms/smart_crusher:** don't crash on a tool call with a null `function`. `_extract_context_from_messages` read `tc.get("function", {})`, but the `{}` default only applies to a missing key — an explicit `{"function": null}` (which some clients emit for a partial/streamed tool call) yields `None`, and the following `.get("arguments")` raised `AttributeError`, taking down `apply()` for the whole request. It now uses `tc.get("function") or {}`, the null-safe form the sibling `_build_tool_name_index` already uses.
